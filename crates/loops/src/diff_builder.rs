...
impl DiffBuilderLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("diff-builder");
        let session = Session::new("diff-builder-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for DiffBuilderLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for DiffBuilderLoop {
    fn loop_type(&self) -> &str { "diff-builder" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("diff-builder", &variables)

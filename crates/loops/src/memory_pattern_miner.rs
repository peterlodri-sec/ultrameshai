...
pub struct MemoryPatternMinerLoop {
    client: LlmClient,
    session: Session,
...
impl MemoryPatternMinerLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("memory-pattern-miner");
        let session = Session::new("memory-pattern-miner-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for MemoryPatternMinerLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for MemoryPatternMinerLoop {
    fn loop_type(&self) -> &str { "memory-pattern-miner" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("memory-pattern-miner", &variables)

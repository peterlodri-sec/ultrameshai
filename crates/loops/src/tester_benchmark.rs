...
pub struct TesterBenchmarkLoop {
    client: LlmClient,
    session: Session,
...
impl TesterBenchmarkLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("tester-benchmark");
        let session = Session::new("tester-benchmark-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for TesterBenchmarkLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for TesterBenchmarkLoop {
    fn loop_type(&self) -> &str { "tester-benchmark" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("tester-benchmark", &variables)

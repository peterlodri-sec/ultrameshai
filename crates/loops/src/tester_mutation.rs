...
impl TesterMutationLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("tester-mutation");
        let session = Session::new("tester-mutation-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for TesterMutationLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for TesterMutationLoop {
    fn loop_type(&self) -> &str { "tester-mutation" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("tester-mutation", &variables)

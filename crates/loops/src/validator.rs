...
impl ValidatorLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("validator");
        let session = Session::new("validator-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for ValidatorLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for ValidatorLoop {
    fn loop_type(&self) -> &str { "validator" }

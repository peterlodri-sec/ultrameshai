...
pub struct MemorySummarizerLoop {
    client: LlmClient,
    session: Session,
...
impl MemorySummarizerLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("memory-summarizer");
        let session = Session::new("memory-summarizer-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for MemorySummarizerLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for MemorySummarizerLoop {
    fn loop_type(&self) -> &str { "memory-summarizer" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("memory-summarizer", &variables)

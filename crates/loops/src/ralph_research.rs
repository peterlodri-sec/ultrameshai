...
impl RalphResearchLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("ralph-research");
        let session = Session::new("ralph-research-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for RalphResearchLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for RalphResearchLoop {
    fn loop_type(&self) -> &str { "ralph-research" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("ralph-research", &variables)

...
impl RalphMetaLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("ralph-meta");
        let session = Session::new("ralph-meta-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for RalphMetaLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for RalphMetaLoop {
    fn loop_type(&self) -> &str { "ralph-meta" }

...
impl QualityStyleLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("quality-style");
        let session = Session::new("quality-style-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for QualityStyleLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for QualityStyleLoop {
    fn loop_type(&self) -> &str { "quality-style" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("quality-style", &variables)

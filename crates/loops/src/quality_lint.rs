...
impl QualityLintLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("quality-lint");
        let session = Session::new("quality-lint-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for QualityLintLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for QualityLintLoop {
    fn loop_type(&self) -> &str { "quality-lint" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("quality-lint", &variables)

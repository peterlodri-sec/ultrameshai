...
impl MathSolverLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("math-solver");
        let session = Session::new("math-solver-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for MathSolverLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for MathSolverLoop {
    fn loop_type(&self) -> &str { "math-solver" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("math-solver", &variables)

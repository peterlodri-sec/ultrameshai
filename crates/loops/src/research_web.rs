...
impl ResearchWebLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("research-web");
        let session = Session::new("research-web-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for ResearchWebLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for ResearchWebLoop {
    fn loop_type(&self) -> &str { "research-web" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("research-web", &variables)

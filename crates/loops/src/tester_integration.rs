...
pub struct TesterIntegrationLoop {
    client: LlmClient,
    session: Session,
...
impl TesterIntegrationLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("tester-integration");
        let session = Session::new("tester-integration-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for TesterIntegrationLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for TesterIntegrationLoop {
    fn loop_type(&self) -> &str { "tester-integration" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("tester-integration", &variables)

...
pub struct QualityTypecheckLoop {
    client: LlmClient,
    session: Session,
...
impl QualityTypecheckLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("quality-typecheck");
        let session = Session::new("quality-typecheck-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for QualityTypecheckLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for QualityTypecheckLoop {
    fn loop_type(&self) -> &str { "quality-typecheck" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("quality-typecheck", &variables)

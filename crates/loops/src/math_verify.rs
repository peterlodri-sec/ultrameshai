...
impl MathVerifyLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("math-verify");
        let session = Session::new("math-verify-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for MathVerifyLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for MathVerifyLoop {
    fn loop_type(&self) -> &str { "math-verify" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("math-verify", &variables)

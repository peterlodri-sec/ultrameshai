...
pub struct CodebaseExplorerLoop {
    client: LlmClient,
    session: Session,
...
impl CodebaseExplorerLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("codebase-explorer");
        let session = Session::new("codebase-explorer-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for CodebaseExplorerLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for CodebaseExplorerLoop {
    fn loop_type(&self) -> &str { "codebase-explorer" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("codebase-explorer", &variables)

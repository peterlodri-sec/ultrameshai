...
pub struct InfraProvisionerLoop {
    client: LlmClient,
    session: Session,
...
impl InfraProvisionerLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("infra-provisioner");
        let session = Session::new("infra-provisioner-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for InfraProvisionerLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for InfraProvisionerLoop {
    fn loop_type(&self) -> &str { "infra-provisioner" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("infra-provisioner", &variables)

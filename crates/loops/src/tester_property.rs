...
impl TesterPropertyLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("tester-property");
        let session = Session::new("tester-property-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for TesterPropertyLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for TesterPropertyLoop {
    fn loop_type(&self) -> &str { "tester-property" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("tester-property", &variables)

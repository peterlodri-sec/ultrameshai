...
impl ResearchDocsLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("research-docs");
        let session = Session::new("research-docs-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for ResearchDocsLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for ResearchDocsLoop {
    fn loop_type(&self) -> &str { "research-docs" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("research-docs", &variables)

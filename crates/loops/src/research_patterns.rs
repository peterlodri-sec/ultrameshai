...
pub struct ResearchPatternsLoop {
    client: LlmClient,
    session: Session,
...
impl ResearchPatternsLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("research-patterns");
        let session = Session::new("research-patterns-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for ResearchPatternsLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for ResearchPatternsLoop {
    fn loop_type(&self) -> &str { "research-patterns" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("research-patterns", &variables)

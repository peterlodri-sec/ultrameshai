...
impl DevopsBuildLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("devops-build");
        let session = Session::new("devops-build-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for DevopsBuildLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for DevopsBuildLoop {
    fn loop_type(&self) -> &str { "devops-build" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("devops-build", &variables)

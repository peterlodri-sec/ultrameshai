...
impl QualityAuditLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("quality-audit");
        let session = Session::new("quality-audit-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for QualityAuditLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for QualityAuditLoop {
    fn loop_type(&self) -> &str { "quality-audit" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("quality-audit", &variables)

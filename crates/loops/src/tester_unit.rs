...
impl TesterUnitLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("tester-unit");
        let session = Session::new("tester-unit-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for TesterUnitLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for TesterUnitLoop {
    fn loop_type(&self) -> &str { "tester-unit" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("tester-unit", &variables)

...
impl NotebookLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("notebook");
        let session = Session::new("notebook-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for NotebookLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for NotebookLoop {
    fn loop_type(&self) -> &str { "notebook" }

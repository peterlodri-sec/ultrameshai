...
pub struct EdgeCaseAnalyzerLoop {
    client: LlmClient,
    session: Session,
...
impl EdgeCaseAnalyzerLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("edge-case-analyzer");
        let session = Session::new("edge-case-analyzer-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for EdgeCaseAnalyzerLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for EdgeCaseAnalyzerLoop {
    fn loop_type(&self) -> &str { "edge-case-analyzer" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("edge-case-analyzer", &variables)

...
impl DevopsPackageLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("devops-package");
        let session = Session::new("devops-package-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for DevopsPackageLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for DevopsPackageLoop {
    fn loop_type(&self) -> &str { "devops-package" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("devops-package", &variables)

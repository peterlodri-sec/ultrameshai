...
impl FixImplementerLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("fix-implementer");
        let session = Session::new("fix-implementer-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for FixImplementerLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for FixImplementerLoop {
    fn loop_type(&self) -> &str { "fix-implementer" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("fix-implementer", &variables)

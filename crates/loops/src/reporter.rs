...
impl ReporterLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("reporter");
        let session = Session::new("reporter-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for ReporterLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for ReporterLoop {
    fn loop_type(&self) -> &str { "reporter" }

...
impl DevopsDeployLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("devops-deploy");
        let session = Session::new("devops-deploy-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for DevopsDeployLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for DevopsDeployLoop {
    fn loop_type(&self) -> &str { "devops-deploy" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("devops-deploy", &variables)

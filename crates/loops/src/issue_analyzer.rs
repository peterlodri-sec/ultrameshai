...
impl IssueAnalyzerLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("issue-analyzer");
        let session = Session::new("issue-analyzer-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for IssueAnalyzerLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for IssueAnalyzerLoop {
    fn loop_type(&self) -> &str { "issue-analyzer" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("issue-analyzer", &variables)

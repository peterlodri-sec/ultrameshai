...
impl FixPlannerLoop {
    pub fn new() -> Self {
        let client = LlmClient::mock("fix-planner");
        let session = Session::new("fix-planner-loop", "unit-000");
        let dispatcher = PromptDispatcher::default();
        Self { client, session, dispatcher, stats: LoopStats::default() }
    }
}

impl Default for FixPlannerLoop {
    fn default() -> Self { Self::new() }
}

#[async_trait::async_trait]
impl Loop for FixPlannerLoop {
    fn loop_type(&self) -> &str { "fix-planner" }
    ...
    async fn process(&mut self, input: LoopInput) -> Result<LoopOutput> {
        ...
        let prompt = self.dispatcher
            .dispatch("fix-planner", &variables)

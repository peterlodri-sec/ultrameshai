...
[![Last updated](https://img.shields.io/github/last-commit/peterlodri-sec/ultrameshai?path=hf-datacard%2FREADME.md&style=for-the-badge&logo=clockify&logoColor=white&label=last%20updated)](https://github.com/peterlodri-sec/ultrameshai/commits/docs/add-math-formulas/hf-datacard/README.md)
...
05 · Notebook sections
       ...
       6.7 Live-sync webhooks (HF + in-process)
       6.8 HF Trusted Publishers
...
> **Last updated:** _auto-refreshed by the `dogfeed` systemd service on every push — see the `last push` row in the HF commit log, or the JSON `created_at` field in `data/latest.jsonl`._

| Metric | Value | Source |
|---|---|---|
| **Loop status** | ![status](https://img.shields.io/badge/status-running-10B981?style=flat-square) | `systemctl is-active dogfeed` |
| **Last updated** | see `last push` in HF commit log | [`commits/main`](https://huggingface.co/datasets/PeetPedro/ultrawhale-dogfood/commits/main) |
...
| 6.7 | [Live-sync webhooks](#67--live-sync-webhooks) | HF settings + NixOS |
| 6.8 | [HF Trusted Publishers](#68--hf-trusted-publishers) | OIDC trust setup |
...
## 6.7 · Live-sync webhooks

When this dataset updates, the proposal site (`proposal.vaked.dev`) re-renders its live ticker. Two webhook channels, both fire on every push:

### A. HuggingFace webhooks (dataset → proposal)

Configure once in [HF user settings → Webhooks](https://huggingface.co/settings/webhooks):

| Field | Value |
|---|---|
| **Target** | `https://proposal.vaked.dev/api/hooks/hf-dogfood` |
| **Watched repos** | `PeetPedro/ultrawhale-dogfood` |
| **Events** | `repo.update` (file added/changed in `data/`) |
| **Secret** | generated by HF, paste into proposal's `DOGFEED_HF_HOOK_SECRET` (sops) |

The proposal Worker:
1. Verifies the `X-Webhook-Secret` header against the sops value.
2. Re-fetches the dataset's `data/stats.json` and `data/latest.jsonl`.
3. Re-renders the live ticker on the proposal site (no full rebuild — just a `cache.put('dogfeed:stats', …)` write).

### B. In-process webhook (dogfeed systemd → proposal)

`packages/dogfeed/src/publish.ts:notifyProposal()` fires a fire-and-forget POST after every successful commit. Configure via NixOS:

```nix
peterlodri.dogfeed.webhookUrl = "https://proposal.vaked.dev/api/hooks/dogfeed";
```

That sets `DOGFEED_WEBHOOK_URL` in the service env. The handler on proposal verifies the source via `X-Dogfeed-Source: ultrameshai` and `X-Dogfeed-Event: batch.pushed` headers, then invalidates the live-ticker cache.

> **Why both?** Channel A is canonical (HF-driven, no trust required — HF signs). Channel B is fast (no HF round-trip). proposal.vaked.dev handles either or both.

---

## 6.8 · HF Trusted Publishers

This dataset ships from a NixOS systemd service using `OIDC` trust via the [HF Trusted Publishers](https://huggingface.co/docs/hub/trusted-publishers) flow — no long-lived HF API tokens in the running service.
...
The net effect: **no long-lived HF tokens in CI, no token rotation, no leak surface**. The only place an HF token lives is on dev-cx53 (sops, mode 0600).
